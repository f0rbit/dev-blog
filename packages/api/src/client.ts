...
const request = async <T>(config: ClientConfig, path: string, options: RequestInit = {}): Promise<FetchResult<T>> => {
...
export const createClient = (config: ClientConfig) => ({
	...
	health: {
		check: (): Promise<FetchResult<{ status: string; timestamp: string; environment: string }>> => request(config, "/health"),
	},
});
