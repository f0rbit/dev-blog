...
const findToken = async (
  ...
  userId: number,
  tokenId: number
): Promise<Result<schema.AccessKey, ApiError>> => {
  const [token] = await db
    .select()
    .from(schema.accessKeys)
    .where(and(
      eq(schema.accessKeys.user_id, userId),
      eq(schema.accessKeys.id, tokenId)
    ))
    .limit(1)
...
tokensRouter.get('/', async (c) => {
  ...
  const tokens = await db
    .select()
    .from(schema.accessKeys)
    .where(eq(schema.accessKeys.user_id, user.id))
...
tokensRouter.put(
  ...
  async (c) => {
    ...
    const updatedRows = await db
      ...
      .set(updates)
      .where(and(
        eq(schema.accessKeys.user_id, user.id),
        eq(schema.accessKeys.id, id)
      ))
      .returning()
