...
const findPostByUuid = async (
  db: ReturnType<typeof drizzle>,
  authorId: number,
  uuid: string
): Promise<Result<schema.PostRow, ApiError>> => {
  const [post] = await db
    .select()
    .from(schema.posts)
    .where(and(
      eq(schema.posts.author_id, authorId),
      eq(schema.posts.uuid, uuid)
    ))
    .limit(1)
...
const getPostTags = async (
  ...
  const tags = await db
    ...
    .from(schema.tags)
    .where(eq(schema.tags.post_id, postId))
...
tagsRouter.put(
  ...
  async (c) => {
    ...
    if (uniqueTags.length > 0) {
      await db
        .insert(schema.tags)
        .values(uniqueTags.map(tag => ({ post_id: postId, tag })))
    }
...
tagsRouter.post(
  ...
  async (c) => {
    ...
    if (newTags.length > 0) {
      await db
        .insert(schema.tags)
        .values(newTags.map(tag => ({ post_id: postId, tag })))
    }
